...
function CfscChurchContent() {
  ...
  useEffect(() => {
    fetch('/api/admin/sunday-guide-units')
      .then((r) => r.json())
      .then((data) => {
        if (data.success) setAllowedUploaders(data.data.units.cfscChurch.allowedUploaders ?? []);
      })
      .catch(() => {});
  }, []);
  ...
  const handleDelete = async (fileId: string, uploaderId?: string) => {
    ...
    try {
      ...
      const qs = new URLSearchParams({ fileId, unitId: 'cfscChurch', userId: currentUserId });
  ...
  const handleRenameTitle = async (fileId: string, newTitle: string) => {
    ...
    try {
      const res = await fetch('/api/sunday-guide/documents', {
        ...
        body: JSON.stringify({ fileId, unitId: 'cfscChurch', userId: user.user_id, sermonTitle: newTitle.trim() }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
  ...
  const handleSelectFile = (fileId: string, fileName?: string) => {
    ...
    const displayName = fileName || (file ? (file.fileName || file.sermonTitle || '') : '');
        ...
        <>
          <div className={`${chatStyles.floatingPanel}${chatOpen ? ' ' + chatStyles.panelOpen : ''}`}>
            <div className={chatStyles.panelHeader}>
              <span className={chatStyles.panelTitle}>CFSC Church AI 助手</span>
