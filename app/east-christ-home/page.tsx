...
function EastChristHomeContent() {
  ...
  useEffect(() => {
    fetch('/api/admin/sunday-guide-units')
      .then((r) => r.json())
      .then((data) => {
        if (data.success) setAllowedUploaders(data.data.units.eastChristHome.allowedUploaders ?? []);
      })
      .catch(() => {});
  }, []);
  ...
  const handleRenameTitle = async (fileId: string, newTitle: string) => {
    ...
    try {
      const res = await fetch('/api/sunday-guide/documents', {
        ...
        body: JSON.stringify({ fileId, unitId: 'eastChristHome', userId: user.user_id, sermonTitle: newTitle.trim() }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
