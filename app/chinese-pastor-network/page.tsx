...
function ChinesePastorNetworkContent() {
  ...
  useEffect(() => {
    fetch('/api/admin/sunday-guide-units')
      .then((r) => r.json())
      .then((data) => {
        if (data.success) setAllowedUploaders(data.data.units.chinesePastorNetwork?.allowedUploaders ?? []);
      })
      .catch(() => {});
  }, []);
  ...
  const handleDelete = async (fileId: string, uploaderId?: string) => {
    ...
    try {
      ...
      const qs = new URLSearchParams({ fileId, unitId: 'chinesePastorNetwork', userId: currentUserId });
  ...
  const handleRenameTitle = async (fileId: string, newTitle: string) => {
    ...
    try {
      const res = await fetch('/api/sunday-guide/documents', {
        ...
        body: JSON.stringify({ fileId, unitId: 'chinesePastorNetwork', userId: user.user_id, sermonTitle: newTitle.trim() }),
      });
      const data = await res.json();
      if (res.ok && data.success) {
        ...
        <>
          <div className={`${chatStyles.floatingPanel}${chatOpen ? ' ' + chatStyles.panelOpen : ''}`}>
            <div className={chatStyles.panelHeader}>
              <span className={chatStyles.panelTitle}>中文牧者網絡 AI 助手</span>
